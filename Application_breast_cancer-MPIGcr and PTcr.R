...
#===================================================
# klapan Meier - original
#=====================================
pdf("Figure_KM-breast_cancer.pdf", width = 16, height = 8.5) 
par(mfrow=c(2,3))

KM0 <- survfit(Surv(tempo, status) ~ 1, data=a2, conf.int=FALSE)
plot(KM0 ,xlab="Time (years)", las=1,cex.axis=1.2,cex.lab=1.2,ylab="Survival Probabily",conf.int=FALSE)

KM1 <- survfit(Surv(tempo, status) ~ ECGRUP, data=a2, conf.int=FALSE)
...
KM2 <- survfit(Surv(tempo, status) ~ CIRURGIA, data=a2)
...
KM3 <- survfit(Surv(tempo, status) ~ RADIO, data=a2)
...
KM4 <- survfit(Surv(tempo, status) ~ QUIMIO, data=a2)
...
vari1 = a2$CIRURGIA*a2$RADIO
vari2 = a2$CIRURGIA*a2$QUIMIO
vari3 = a2$RADIO*a2$QUIMIO
vari4 = a2$CIRURGIA*a2$RADIO*a2$QUIMIO

KM50 <- survfit(Surv(tempo, status) ~ vari1+vari2+vari3+vari4, data=a2)
...
#------------------------------------
#  log_like, AIC and BIC for  MPIGcr
maximo.logMIG = llikeobserved(psi.MIG,Data=Data,dist =dist,model=1);maximo.logMIG 
AIC_MIG       = 2*(maximo.logMIG) + 2*length(psi.MIG);AIC_MIG
BIC_MIG       = 2*(maximo.logMIG) + log(length(t))*length(psi.MIG);BIC_MIG
#----------------------------------------------------
#  hessian do PRACMA - MPIGcr
hessian.psi.MIG    = sqrt(diag(solve(hessian(llikeobserved,x0=psi.MIG,Data=Data,dist=1,model=1))));hessian.psi.MIG
se.hessian.psi.MIG = matrix(hessian.psi.MIG,ncol=r+3,nrow=r+3);
estimates.psi.MIG  = matrix(c(psi.MIG,round(hessian.psi.MIG,6)),ncol=2);
colnames(estimates.psi.MIG) = c("Estimate_MIG","SE_hessian_MIG");rownames(estimates.psi.MIG) = c(paste("beta",0:(r-1),sep=""),"alpha","nu","m")


#------------------------------------
#matriz de coefientes c/ p-valor
tval_MIG = estimates.psi.MIG[1:r,1]/ estimates.psi.MIG[1:r,2]
matcoef_MIG = cbind( estimates.psi.MIG[1:r,1],  estimates.psi.MIG[1:r,2], tval_MIG, 2*(1-pnorm(abs(tval_MIG))))
dimnames(matcoef_MIG) = list(names(tval_MIG), c(" Estimate_MIG"," Std. Error", " t value", "Pr(>|t|)"))
cat("\nCoefficient(s):\n")
printCoefmat(matcoef_MIG, digits = 6, signif.stars = TRUE)
...
name2 = paste("Table-application_results")
...
psi_PO = EM_PO$estimate
#  log.likelihood estimated - Poisson
llikeEstimated_PO <- EM_PO$loglike;llikeEstimated_PO
#criteria
AIC_PO = EM_PO$AIC;AIC_PO
BIC_PO = EM_PO$BIC;BIC_PO
BF_PO = 2*(-maximo.logMIG - (-llikeEstimated_PO))  - (length(psi.MIG) - length(psi_PO[,1]) ) *log(n)
#Coefficient matrix with P-values
tval3 = psi_PO[1:r,1]/psi_PO[1:r,2]
matcoef3 = cbind(psi_PO[1:r,1], psi_PO[1:r,2], tval3, 2*(1-pnorm(abs(tval3))))
dimnames(matcoef3) = list(names(tval3), c(" Estimate_PO"," Std. Error", " t value", "Pr(>|t|)"))
cat("\nCoefficient(s):\n")
mat_coef_PO = printCoefmat(matcoef3, digits = 6, signif.stars = TRUE)
...
#----------------------------------
#cenario 1.1 
# 20 years; Stage 1, Radio e quimio=SIM
...
#----------------------------------
#cenario 1.2
# 20 years; Stage 2, Radio e quimio=SIM
...
#----------------------------------
#cenario 1.3
# 20 years; Stage 3, Radio e quimio=SIM
...
#cenario 1.4
# 20 years; Stage 4, Radio e quimio=SIM
...
#----------------------------------
#cenario 3.1 
# 60years; Stage 1, Radio e quimio=SIM
...
#----------------------------------
#cenario 3.2
# 60 years; Stage 2, Radio e quimio=SIM
...
#----------------------------------
#cenario 3.3
# 60 years; Stage 3, Radio e quimio=SIM
...
#cenario 3.4
# 60 years; Stage 4, Radio e quimio=SIM
...
#graf. sobrev. estimada perfil 1 e 2
w_in <- 1018/96
h_in <- 350/96
cairo_pdf("Figure_qqplot_estimated_survival_hazard.pdf", width = w_in, height=h_in)
...
pcure_rates = round(rbind( p_surg_20 , p_radio_20 , p_chemo_20,p_surg_radio_20,
                          p_surg_chemo_20,p_radio_chemo_20,p_surg_radio_chemo_20,
                           p_surg_60 , p_radio_60 , p_chemo_60,p_surg_radio_60,
                          p_surg_chemo_60,p_radio_chemo_60,p_surg_radio_chemo_60),3)
dimnames(pcure_rates) = list( c("20y_Surgery", 
                                "20y_Radiotherapy","20y_Chemotherapy", "20y_Surg+Radio",
                                "20y_Surg+Chemo", "20y_Radio+Chemo",
                                "20y_Surg+Radio+Chemo",
                                "60y_Surgery", 
                                "60y_Radiotherapy","60y_Chemotherapy", "60y_Surg+Radio 60years",
                                "60y_Surg+Chemo", "60y_Radio+Chemo",
                                "60y_Surg+Radio+Chemo"),
                              c("Stage 1"," Stage 2", "Stage 3", "Stage 4"))
pcure_rates
name13 = paste("Table_ML-estimates_of_p_cure_rate_Breast_cancer_data")
...
ICcure_rates = round(rbind(
#  IC_noTreat_20, 
  IC_surg_20 , IC_radio_20 , IC_chemo_20,IC_surg_radio_20,
                           IC_surg_chemo_20,IC_radio_chemo_20,IC_surg_radio_chemo_20,
                              IC_surg_60 , IC_radio_60 , IC_chemo_60,IC_surg_radio_60,
                           IC_surg_chemo_60,IC_radio_chemo_60,IC_surg_radio_chemo_60),3)
dimnames(ICcure_rates) = list( c("20y_Surgery", 
                                 "20y_Radiotherapy","20y_Chemotherapy", "20y_Surg+Radio",
                                 "20y_Surg+Chemo", "20y_Radio+Chemo",
                                 "20y_Surg+Radio+Chemo",
                                "60y_Surgery", 
...
ICcure_rates
name14 = paste("Table_ML-estimates_of_Confidence_Interval-cure_rate_Breast_cancer_data")
...
cairo_pdf("Figure_CURE_RATES.pdf", width = w_in, height = h_in)
...
ggplot(
  ...
  theme(
    ...
    legend.background = element_blank(),  # 👈 remove o quadrado
...
theta2_mig <- exp(t(x2)%*%beta.est_MIG) 
p2_mig = m.est*exp((1-m.est)*(1-sqrt(1+(2*m.est*theta2_mig)/(1-m.est)))) + 
  (1-m.est)*exp((((m.est+1)*(1-m.est^2))/(m.est^2))*(1-sqrt(1+(2*m.est^2*theta2_mig)/(1-m.est^2)) ))
...
theta2_po <- exp(t(x2)%*%beta.est_PO) 
p2_po = 1/exp(theta2_po)
...
#=======================================
# tabela cure rates, IC e diferenca percentual
# Poisson e MIG
#=========================================
...
# IC em matriz (lwr,upr) por estágio
...
write.table(tab_out,
            file = "cure_rate_by_stage_PT_MPIG.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)

write.csv(tab_out,
          file = "cure_rate_by_stage_PT_MPIG.csv",
          row.names = FALSE)

# =========================
# Dumbbell plot com IC (PO vs MIG) por Stage
# =========================
pcure_PO  <- c(p1_po, p2_po, p3_po, p4_po)
pcure_MIG <- c(p1_mig, p2_mig, p3_mig, p4_mig)
IC_pcure_PO  <- c(ICp1_po, ICp2_po, ICp3_po, ICp4_po)
...
w_in <- 1018/96
h_in <- 300/96
cairo_pdf("diff_cure_rate.pdf", width = w_in, height=h_in)
...
# to generate the results of figures and tables of manuscript
save(list = ls(),file = "aplication_breast_cancer_MIG_POISSON.RData")
#######################################################################
